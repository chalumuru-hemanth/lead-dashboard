...
// ---------------------------------------------------------------------------
// Outreach / email triage (Apps Script bridge + Gemini) helpers.
// ---------------------------------------------------------------------------

export const PRIORITY_LABEL = { high: "High", medium: "Medium", low: "Low" };
export const PRIORITY_COLOR = { high: "#FB7185", medium: "#FBBF24", low: "#5C5F80" };

export function hasReply(row) {
  return !!(row && (row.repliedAt || String(row.status || "").toLowerCase() === "replied"));
}

// A reply needs (re-)triage if it's never been classified, or if it was
// replied to again after the last time AI processed it.
export function needsTriage(row) {
  if (!hasReply(row)) return false;
  if (!row.aiPriority) return true;
  if (!row.aiProcessedAt) return true;
  if (!row.repliedAt) return false;
  return new Date(row.repliedAt).getTime() > new Date(row.aiProcessedAt).getTime();
}

// "Needs attention" = replied, and not explicitly deprioritized by the AI
// (covers high/medium and anything not yet classified).
export function needsAttention(row) {
  return hasReply(row) && row.aiPriority !== "low";
}

export function emailContactName(row) {
  return (row && row.firstName) || "Unknown";
}

// ---------------------------------------------------------------------------
// Call quality — distinguishes real prospect conversations from noise
// (failed connections, calls that ended before anything was said, calls
// that haven't been through analysis yet) so the Leads view can surface
// signal instead of a table full of empty cells.
// ---------------------------------------------------------------------------

export const CALL_QUALITY_LABEL = {
  real: "Analyzed",
  unanalyzed: "Not yet analyzed",
  short: "Too short to analyze",
  failed: "Failed to connect",
};

export const CALL_QUALITY_COLOR = {
  real: "#10B981",
  unanalyzed: "#F59E0B",
  short: "#94A3B8",
  failed: "#F43F5E",
...
export function callQuality(c) {
  if (!c) return "failed";
  if (c.durationSeconds == null || /error/i.test(c.endedReason || "")) return "failed";
  if (c.durationSeconds < 15) return "short";
  if (!c.hasStructuredData && !c.summaryText) return "unanalyzed";
  return "real";
}

// One scannable line of insight for a lead row, regardless of how complete
// the structured extraction is -- falls back gracefully so a row is never
// just a wall of "—".
export function leadHeadline(c) {
  if (callQuality(c) === "failed") {
    return (c.endedReason || "").replace(/-/g, " ") || "Call failed to connect.";
  }
  if (callQuality(c) === "short") {
    return "Call ended within seconds — likely a hang-up or wrong number.";
  }
  if (c.summaryText) return c.summaryText;
  if (callQuality(c) === "unanalyzed") return "Analysis pending — check back shortly, or open this call to generate it now.";
