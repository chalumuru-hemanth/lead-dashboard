...
// ---- List-route enrichment -------------------------------------------

// Vapi's own analysis doesn't always land on a call (deprecated
// analysisPlan can be flaky, and the newer Structured Outputs migration
// only applies going forward). Supabase already has the clean Gemini
// summary + structured data for any call that's been opened in the
// dashboard or swept up by the /api/sync backfill -- so the lightweight
// list route can borrow that instead of showing a wall of empty cells for
// calls Vapi itself hasn't (yet) analyzed. Best-effort: on any failure,
// callers get back exactly what Vapi gave them, unchanged.
export async function enrichCallsFromSupabase(calls) {
  const sb = getSupabase();
  if (!sb || !calls || !calls.length) return calls;
  try {
    const ids = calls.map((c) => c.id);
    const { data, error } = await sb
      .from("vapi_calls")
      .select("id, summary, structured_data")
      .in("id", ids);
    if (error) throw error;
    ...
    return calls.map((c) => {
      ...
      if (!stored) return c;
      const summaryText = c.summaryText || stored.summary || null;
      if (!c.hasStructuredData && stored.structured_data) {
        const sd = stored.structured_data;
        return {
          ...c,
          summaryText,
          hasStructuredData: true,
          contact: sd.contact || null,
          intent: sd.intent || null,
          outcome: sd.outcome || null,
          sentiment: sd.sentiment || null,
          prior_auth: sd.prior_auth || null,
          qualification: sd.qualification || null,
          follow_up: sd.follow_up || null,
        };
      }
      return { ...c, summaryText };
    });
  } catch (err) {
    await logSync("call_enrich", "error", err.message);
    return calls;
  }
}
